...
package body Gabyx.Drivers.Raylib.Settings.Pane_Window is
   ...
   function Get_Preset_Label (Index : Positive) return String is
     (case Index is
         when 1 => "1280 x 720 (16:9 HD)",
         when 2 => "1440 x 900 (16:10 WXGA)",
         when 3 => "1600 x 900 (16:9 HD+)",
         when 4 => "1920 x 1080 (16:9 FHD)",
         when 5 => "1920 x 1200 (16:10 WUXGA)",
         when 6 => "2560 x 1080 (21:9 UW-HD)",
         when 7 => "2560 x 1440 (16:9 QHD)",
         when 8 => "3440 x 1440 (21:9 UW-QHD)",
         when 9 => "3840 x 2160 (16:9 4K UHD)",
         when others => "Nieznany Preset");

   function Get_Zoom_Label (Index : Positive) return String is
     (case Index is
         when 1 => "24 px",
         when 2 => "32 px",
         when 3 => "48 px",
   ...
   begin
      ...
      Gabyx.Drivers.Raylib.Widgets.Draw_Section_Box
        (X => Pane_X, Y => Pane_Y + 215, W => Pane_W, H => 75,
         Title => "2. SKALOWANIE INTERFEJSU HUD (PROFILE)",
         Font => Cur_Font, Font_Size => F_Size);
      ...
      Gabyx.Drivers.Raylib.Widgets.Draw_Section_Box
        (X => Pane_X, Y => Pane_Y + 300, W => Pane_W, H => 75,
         Title => "3. SKALOWANIE SIATKI SWIATA (ZOOM KAFELKA)",
         Font => Cur_Font, Font_Size => F_Size);

      Gabyx.Drivers.Raylib.Widgets.Draw_Cycle_Selector
        (X => Pane_X + 15, Y => Pane_Y + 332, W => Pane_W - 30,
         Label => "Rozmiar kafelka:",
         Value_Text => Get_Zoom_Label (Staged_Zoom_Idx),
         Font => Cur_Font, Font_Size => F_Size,
         Prev_Clicked => Prev_Z, Next_Clicked => Next_Z);
      ...
      Gabyx.Drivers.Raylib.Widgets.Draw_Button
        (X => Pane_X + Pane_W - 270, Y => Pane_Y + 390, W => 270, H => 40,
